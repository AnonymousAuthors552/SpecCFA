...
while last == 0 and app == 1:
	...
	rcf = 0
	rcf_start = time.perf_counter()
	prv_cflog = ser.read(log_size)
	rcf_stop = time.perf_counter()
	rcf = rcf_stop-rcf_start
	...
	print("\t\tAttest: "+str(time_attest), file=debugfile)
	...
	runtime_rounds.append([time_attest, time_recv_resp, total_s4, time_gen_resp, time_send_resp, time_closing_steps, rcf])
	print(" ", file=debugfile)
	#####--------------------------------------
...
print("--------------------------------------------------", file=debugfile)
print("Protocol Run Time ("+str(report_num)+" reports)", file=debugfile)
total_time = [0, 0, 0, 0, 0, 0, 0]
print("Round | Attest | Send resp. | Step 4 | Step 5 | Step 6 | Step 7-9 | Recv CFLog", file=debugfile)
for i in range(0, len(runtime_rounds)):
	...
	for j in range(0, len(runtime_rounds[i])):
		...
		total_time[j] += runtime_rounds[i][j]
	print("", file=debugfile)
print("--------------------------------------------------", file=debugfile)
print(" ", file=debugfile)
print("--------------------------------------------------", file=debugfile)
for j in range(0, len(total_time)):
	print("%s   " % str(total_time[j]), end='', file=debugfile)
print("--------------------------------------------------")
